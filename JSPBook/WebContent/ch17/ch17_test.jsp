                    ...
                    <div class="content-left wow fadeInLeft" data-wow-delay=".3s">
                    	<!-- 
                    	 ...
                    	 <c:forEach items="${names }" var="nm" varStatus="stat">
                    	 	<c:choose>
                    	 		<c:when test="${fn:substring(names[stat.index], 0, 1) == '김' }">
                    	 			<c:set var="kimCnt" value="${kimCnt + 1 }"/>
                    	 		</c:when>
                    	 		<c:when test="${fn:substring(names[stat.index], 0, 1) == '강' }">
                    	 			<c:set var="kangCnt" value="${kangCnt + 1 }"/>
                    	 		</c:when>
                    	 		<c:when test="${fn:substring(names[stat.index], 0, 1) == '민' }">
                    	 			<c:set var="minCnt" value="${minCnt + 1 }"/>
                    	 		</c:when>
                    	 		<c:when test="${fn:substring(names[stat.index], 0, 1) == '박' }">
                    	 			<c:set var="parkCnt" value="${parkCnt + 1 }"/>
                    	 		</c:when>
                    	 		<c:when test="${fn:substring(names[stat.index], 0, 1) == '서' }">
                    	 			<c:set var="seoCnt" value="${seoCnt + 1 }"/>
                    	 		</c:when>
                    	 		<c:when test="${fn:substring(names[stat.index], 0, 1) == '송' }">
                    	 			<c:set var="songCnt" value="${songCnt + 1 }"/>
                    	 		</c:when>
                    	 		<c:when test="${fn:substring(names[stat.index], 0, 1) == '이' }">
                    	 			<c:set var="leeCnt" value="${leeCnt + 1 }"/>
                    	 		</c:when>
                    	 		<c:when test="${fn:substring(names[stat.index], 0, 1) == '장' }">
                    	 			<c:set var="jangCnt" value="${jangCnt + 1 }"/>
                    	 		</c:when>
                    	 		<c:when test="${fn:substring(names[stat.index], 0, 1) == '전' }">
                    	 			<c:set var="jeonCnt" value="${jeonCnt + 1 }"/>
                    	 		</c:when>
                    	 		<c:when test="${fn:substring(names[stat.index], 0, 1) == '정' }">
                    	 			<c:set var="jeongCnt" value="${jeongCnt + 1 }"/>
                    	 		</c:when>
                    	 		<c:when test="${fn:substring(names[stat.index], 0, 1) == '최' }">
                    	 			<c:set var="choiCnt" value="${choiCnt + 1 }"/>
                    	 		</c:when>
                    	 		<c:when test="${fn:substring(names[stat.index], 0, 1) == '한' }">
                    	 			<c:set var="hanCnt" value="${hanCnt + 1 }"/>
                    	 		</c:when>
                    	 	...
                    	 	<table class="table table-bordered">
                    	 		<tr>
                    	 			<td>김</td><td>강</td><td>민</td><td>박</td><td>서</td><td>송</td><td>이</td>
                    	 			<td>장</td><td>전</td><td>정</td><td>최</td><td>한</td><td>홍</td>
                    	 		</tr>
                    	 		<tr>
                    	 			<td>${kimCnt }</td>
                    	 			<td>${kangCnt }</td>
